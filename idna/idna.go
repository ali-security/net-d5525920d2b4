...
func (p *Profile) process(s string, toASCII bool) (string, error) {
	...
	for ; !labels.done(); labels.next() {
		...
		if strings.HasPrefix(label, acePrefix) {
			...
			// A Punycode payload never decodes to a non-empty all-ASCII
			// label: the encoder only emits an extended part for runes
			// >= U+0080 and the decoder only ever inserts runes >= U+0080,
			// so an all-ASCII result means the label was hand-crafted.
			// Accepting one lets "xn--example-.com" convert to
			// "example.com", so a caller that vets the ASCII form of a host
			// before converting it can be made to admit a name it already
			// rejected. UTS #46 revision 35 requires rejecting these
			// (error P4). The check is deliberately not gated on unicode16:
			// the flaw exists for every version of the mapping tables, and
			// gating it would leave the module vulnerable whenever it is
			// built with a toolchain whose unicode.Version is below 16.
